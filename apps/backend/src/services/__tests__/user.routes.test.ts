...
describe('User Management API Endpoints (Admin)', () => {
  ...
  describe('GET /api/v1/users', () => {
    it('should allow an admin to get a list of all active users', async () => {
  ...
  describe('DELETE /api/v1/users/:id (Soft Delete)', () => {
    it('should allow an admin to soft delete a user', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.message).toContain('User archived successfully');

      // Verifikasi bahwa user masih ada di database (paranoid: false)
