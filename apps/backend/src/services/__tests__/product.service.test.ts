...
describe("ProductService", () => {
  ...
  describe("createProduct", () => {
    it("should create a new product successfully", async () => {
      ...
      const createdProduct = {
        ...createdProductJSON,
        toJSON: () => createdProductJSON,
      };

      mockProduct.create.mockResolvedValue(createdProduct as any);
      mockProduct.findByPk.mockResolvedValue(createdProduct as any); // Add this line
  ...
  describe("getAllProducts", () => {
    it("should call APIFeatures and return paginated products", async () => {
      const mockProducts = [
        { name: "Product 1", toJSON: () => ({ name: "Product 1" }) },
        { name: "Product 2", toJSON: () => ({ name: "Product 2" }) },
      ];

      // PERBAIKAN DI SINI: Menambahkan search() ke dalam mock
      (mockAPIFeatures as any).mockImplementation(() => ({
        search: jest.fn().mockReturnThis(), // <--- Tambahan Baru
  ...
  describe("getProductsBySeller", () => {
    it("should correctly filter products by sellerId", async () => {
      const sellerId = "seller-uuid";
      const mockProducts = [
        { name: "My Product 1", toJSON: () => ({ name: "My Product 1" }) },
      ];
  ...
  describe("updateProduct", () => {
    ...
    it("should update a product successfully if user is the owner", async () => {
      ...
      expect(mockExistingProduct.update).toHaveBeenCalledWith(
        updateData,
        expect.anything()
      );
  ...
  describe("getProductById", () => {
    ...
    it("should return a product if found", async () => {
      const mockProductData = {
        id: productId,
        name: "Found Product",
        toJSON: () => ({ id: productId, name: "Found Product" }),
      };
