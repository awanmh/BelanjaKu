...
describe("Cart Service", () => {
  ...
  describe("addToCart", () => {
    it("should add new item to cart successfully if product exists and stock is sufficient", async () => {
      ...
      expect(db.CartItem.findOne).toHaveBeenCalledWith({
    ...
    it("should throw error if product not found", async () => {
      ...
      // ACT & ASSERT
      await expect(
        cartService.addToCart(mockUserId, mockProductId, 1)
      ).rejects.toThrow(new HttpException(404, "Product not found"));
    });
    ...
    it("should throw error if stock is insufficient", async () => {
      ...
      // ACT & ASSERT
      await expect(
        cartService.addToCart(mockUserId, mockProductId, 10)
      ).rejects.toThrow(/Insufficient stock/); // Cek sebagian pesan error
    });
