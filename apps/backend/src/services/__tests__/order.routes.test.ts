...
describe('Order API Endpoints', () => {
  ...
  beforeAll(async () => {
    ...
    const sellerUser = await db.User.findOne({ where: { email: 'seller.order@test.com' } });
    await sellerUser!.update({ role: 'seller' });
    sellerId = sellerUser!.id;
    const category = await db.Category.create({ name: 'Electronics' });
    const product1 = await db.Product.create({ name: 'Mouse', description: 'A nice mouse', price: 150000, stock: 20, sellerId, categoryId: category.id, imageUrl: 'mouse.jpg' });
    const product2 = await db.Product.create({ name: 'Keyboard', description: 'A nice keyboard', price: 300000, stock: 15, sellerId, categoryId: category.id, imageUrl: 'keyboard.jpg' });
  ...
  describe('POST /api/v1/orders', () => {
    ...
    it('should return 400 Bad Request for an invalid promotion code', async () => {
        ...
        expect(response.body.message).toContain('Invalid or expired promotion code');
    });
  });
});