...
export const addToCart = async (userId: string, productId: string, quantity: number) => {
    ...
    if (newQuantity > product.stock) {
        throw new HttpException(400, `Insufficient stock. Only ${product.stock} left.`);
    }
...
export const updateCartItem = async (userId: string, cartItemId: string, quantity: number) => {
    ...
    if (quantity > cartItem.product.stock) {
        throw new HttpException(400, `Insufficient stock. Max available: ${cartItem.product.stock}`);
    }
