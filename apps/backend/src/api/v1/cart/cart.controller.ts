...
export const getMyCart = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
...
export const addItem = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        res.status(201).json({ status: 'success', message: 'Item added to cart', data });
    } catch (error) {
        next(error);
    }
};
...
export const updateItem = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        ...
        const data = await cartService.updateCartItem(userId, id, quantity);
...
export const clearAll = async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
        const userId = req.user!.id;
