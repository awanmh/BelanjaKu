  ...
  const generateMockProducts = (): FlashSaleProduct[] => {
    return [
      ...
      },
      {
        id: "m4",
        brand: "Adidas",
        article: "Samba OG White",
        price: 2200000,
        originalPrice: 2500000,
        discount: 12,
        imageUrl:
          "https://images.unsplash.com/photo-1549298916-b41d501d3772?q=80&w=800&auto=format&fit=crop",
        seller: { fullName: "Sneaker Head" },
        rating: "4.9",
        sold: 15,
        totalStock: 30,
      },
    ...
    const fetchFlashSale = async () => {
      ...
      try {
        ...
        const flashSaleData: FlashSaleProduct[] = rawProducts.map((p: any) => {
          ...
          const discount =
            discountNum ?? [20, 35, 50, 60][Math.floor(Math.random() * 4)];

          const originalPrice =
            Number(p.originalPrice ?? priceNum / (1 - discount / 100)) ||
            priceNum;
          ...
          return {
            ...
            rating: p.rating
              ? String(Number(p.rating).toFixed(1))
          ...
          <div className="flex items-center gap-3">
            {["hours", "minutes", "seconds"].map((unit, idx) => (
              <div key={unit} className="flex items-center gap-3">
                ...
                {idx < 2 && (
                  <span className="font-bold text-2xl text-gray-300 mb-4">
                    :
                  </span>
                )}
              </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-[450px] bg-gray-100 rounded-2xl animate-pulse"
            />
          ))}
        </div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6">
          {products.map((item) => {
            const progressPercent = Math.min(
              100,
              Math.round((item.sold / Math.max(1, item.totalStock)) * 100)
            );
            ...
            return (
              <Link key={item.id} href={`/products/${item.id}`}>
                <div className="bg-white rounded-2xl p-4 shadow-md hover:shadow-xl hover:-translate-y-1 transition-all duration-300 border border-gray-100 flex flex-col relative overflow-hidden group cursor-pointer">
                  {/* Seller */}
