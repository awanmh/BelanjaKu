...
export type CreateShippingOptionInput = Pick<ShippingOptionAttributes, 'name' | 'description' | 'price' | 'estimatedDays'>;

// Tipe data spesifik untuk input pembaruan opsi pengiriman
export type UpdateShippingOptionInput = Partial<CreateShippingOptionInput & { isActive: boolean }>;
...
class ShippingOptionService {
   ...
   */
  public async create(data: CreateShippingOptionInput): Promise<ShippingOptionAttributes> {
    const { name } = data;

    // Cek apakah nama opsi pengiriman sudah ada
    const existingOption = await ShippingOption.findOne({ where: { name } });
    if (existingOption) {
      throw new HttpException(StatusCodes.CONFLICT, 'Shipping option with this name already exists');
    }
  ...
  public async findById(id: string): Promise<ShippingOptionAttributes> {
    ...
    if (!option) {
      throw new HttpException(StatusCodes.NOT_FOUND, 'Shipping option not found');
    }
   ...
   */
  public async update(id: string, data: UpdateShippingOptionInput): Promise<ShippingOptionAttributes> {
    const option = await ShippingOption.findByPk(id);
    if (!option) {
      throw new HttpException(StatusCodes.NOT_FOUND, 'Shipping option not found');
    }
