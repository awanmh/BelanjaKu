...
export type CreatePromotionInput = Omit<PromotionAttributes, 'id' | 'isActive' | 'createdAt' | 'updatedAt'>;
...
class PromotionService {
   ...
   */
  public async create(data: CreatePromotionInput, sellerId: string): Promise<PromotionAttributes> {
    const { productId, code } = data;

    // Verifikasi bahwa produk tersebut milik penjual yang sedang login
    const product = await Product.findByPk(productId);
    if (!product || product.sellerId !== sellerId) {
      throw new HttpException(StatusCodes.FORBIDDEN, 'You can only create promotions for your own products.');
    }

    if (code) {
      const existingPromo = await Promotion.findOne({ where: { code } });
      if (existingPromo) {
        throw new HttpException(StatusCodes.CONFLICT, 'Promotion code already exists.');
      }
    }
   ...
   */
  public async update(id: string, data: UpdatePromotionInput, sellerId: string): Promise<PromotionAttributes> {
    const promotion = (await Promotion.findByPk(id, { include: [{ model: Product, as: 'product' }] })) as PromotionWithProduct | null;
    ...
    if (promotion.product?.sellerId !== sellerId) {
        throw new HttpException(StatusCodes.FORBIDDEN, 'You are not authorized to update this promotion.');
    }
  ...
  public async delete(id: string, sellerId: string): Promise<void> {
    ...
    // Verifikasi kepemilikan
    if (promotion.product?.sellerId !== sellerId) {
        throw new HttpException(StatusCodes.FORBIDDEN, 'You are not authorized to delete this promotion.');
    }
