...
describe('PromotionService', () => {
  ...
  describe('create', () => {
    ...
    it('should throw FORBIDDEN if product does not belong to the seller', async () => {
      mockProduct.findByPk.mockResolvedValue({ sellerId: 'another-seller' } as any);

      await expect(PromotionService.create(promotionData, sellerId)).rejects.toThrow(
        new HttpException(StatusCodes.FORBIDDEN, 'You can only create promotions for your own products.')
      );
    });
    ...
    it('should throw CONFLICT if promo code already exists', async () => {
      mockProduct.findByPk.mockResolvedValue({ sellerId } as any);
      mockPromotion.findOne.mockResolvedValue(mockPromotionInstance as any); // Kode ditemukan

      await expect(PromotionService.create(promotionData, sellerId)).rejects.toThrow(
        new HttpException(StatusCodes.CONFLICT, 'Promotion code already exists.')
      );
    });
  });
  ...
  describe('update', () => {
    it('should update a promotion successfully', async () => {
        mockPromotion.findByPk.mockResolvedValue(mockPromotionInstance as any);
        await PromotionService.update('promo-uuid', { isActive: false }, sellerId);
        expect(mockPromotion.findByPk).toHaveBeenCalledWith('promo-uuid', expect.any(Object));
        expect(mockPromotionInstance.update).toHaveBeenCalledWith({ isActive: false });
    });

    it('should throw FORBIDDEN if user is not the owner', async () => {
        mockPromotion.findByPk.mockResolvedValue(mockPromotionInstance as any);
        await expect(PromotionService.update('promo-uuid', {}, 'another-seller')).rejects.toThrow(
            new HttpException(StatusCodes.FORBIDDEN, 'You are not authorized to update this promotion.')
        );
    });
    ...
    it('should throw NOT_FOUND if promotion to update is not found', async () => {
      mockPromotion.findByPk.mockResolvedValue(null);
      await expect(PromotionService.update('promo-uuid', {}, sellerId)).rejects.toThrow(
        new HttpException(StatusCodes.NOT_FOUND, 'Promotion not found')
      );
    });
  });
  ...
  describe('delete', () => {
    it('should delete a promotion successfully', async () => {
     ...
     it('should throw FORBIDDEN if user is not the owner', async () => {
        mockPromotion.findByPk.mockResolvedValue(mockPromotionInstance as any);
        await expect(PromotionService.delete('promo-uuid', 'another-seller')).rejects.toThrow(
            new HttpException(StatusCodes.FORBIDDEN, 'You are not authorized to delete this promotion.')
        );
    });
    ...
    it('should throw NOT_FOUND if promotion to delete is not found', async () => {
      mockPromotion.findByPk.mockResolvedValue(null);
      await expect(PromotionService.delete('promo-uuid', sellerId)).rejects.toThrow(
        new HttpException(StatusCodes.NOT_FOUND, 'Promotion not found')
      );
    });
  });
